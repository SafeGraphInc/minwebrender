...
- **Internal-only**: ClusterIP, no ingress. Callers use
  `http://minwebrender.<namespace>.svc.cluster.local:10000`.
- **No AWS permissions**: the renderer touches no S3, RDS, or Secrets Manager,
  so it needs no IRSA role and no secrets.
- **CI test**: boots the built image and renders `example.com` end-to-end. That
  is the check that catches Chromium/playwright version skew and a browser
  that will not launch under the container's seccomp profile.
...
- `--no-sandbox --disable-dev-shm-usage`, needed under a pod's seccomp profile
  and 64Mi `/dev/shm`.
- Gunicorn pinned to one worker: one worker means one Chromium. A second worker
  process would mean a second browser and double the memory for no extra
  throughput. Scale with replicas.
- Not exposed to the internet. `minwebrender.net` was an unauthenticated open
  renderer — anyone could make SafeGraph infrastructure fetch any URL.
